...
export const updatedUserData = async (req, res) => {
  try {
    ...
    res.json({ success: true, user, message: "updated successfully" });
  } catch (error) {
    console.log(error);
    res.json({ success: false, message: error.message });
  }
...
export const followUser = async (req, res) => {
  try {
    ...
    await toUser.save();
    res.json({ success: true, message: "Now you Body.json(): Promise<any>" });
  } catch (error) {
    console.log(error);
    res.json({ success: false, message: error.message });
  }
...
export const sendConnectionRequest = async (req, res) => {
  try {
    ...
    if (!connection) {
      ...
      return res.json({
        success: false,
        message: "You have sent more than 20 connection requests in last 24",
      });
    }
